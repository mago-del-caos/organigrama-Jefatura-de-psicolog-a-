...
// 4. LÓGICA DE FLUJO DE TRABAJO (Corregida estilo árbol genealógico)
...
document.getElementById('calc-ruta').addEventListener('click', () => {
    ...
    // D3 path() calcula la ruta subiendo al ancestro común y bajando
    ...
    path.forEach((nodo, index) => {
        ...
        if(index < path.length - 1) {
            ...
            if (nextDepth < currDepth) {
                arrow = "⬆️"; // Sube en la jerarquía
            } else if (nextDepth > currDepth) {
                arrow = "⬇️"; // Baja en la jerarquía
            }
...
darkModeToggle.addEventListener('click', (e) => {
    ...
    if (document.body.classList.contains('dark-mode')) {
        e.target.textContent = '☀️ Modo Claro';
...
// 6. PWA LÓGICA (Funciona en PC y Móvil)
let deferredPrompt;
...
installBtn.addEventListener('click', async () => {
    if (deferredPrompt) {
        ...
        alert("Para instalar la app en PC: Ve al menú de tu navegador (Chrome/Edge) y selecciona 'Instalar esta aplicación' o haz clic en el icono de instalación en la barra de direcciones.");
    }
});
